...
// 构造函数
Channel::Channel(int fd, Epoll *epoll, bool isListen) : m_fd(fd), m_epoll(epoll), m_isListen(isListen) {
}
...
void Channel::handleEvent(Socket *listen_socket) {
    if (m_revents & EPOLLRDHUP) {                   // 如果是客户端关闭(或者recv返回0)
        ...
        if (m_isListen) {   // 如果是监听的Channel,表示新连接
            ...
            Channel *server_channel = new Channel(client_socket->getFd(), m_epoll, false);  // 创建clientfd的Channel对象
            server_channel->enableReading();                                                // 将listnefd的Channel对象设置为可读
            server_channel->setEdgeTriggered();                                             // 设置为边缘触发
        }
        else {              // 如果是客户端的Channel,表示有数据可读
            char buffer[1024];
